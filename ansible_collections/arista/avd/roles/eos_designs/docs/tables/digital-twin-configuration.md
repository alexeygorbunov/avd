...
=== "Table"
    ...
    | [<samp>&nbsp;&nbsp;&nbsp;&nbsp;enabled</samp>](## "digital_twin.endpoints.enabled") | Boolean |  | `False` |  | Setting this flag to `true` will include all `connected_endpoints` in the generated Digital Twin topology data.<br>Specific ednpoints may be suppressed by setiing `<connected_endpoints_keys.key>.digital_twin.enabled` to `false`. |
...
=== "YAML"
    ...
    digital_twin:
      ...
      endpoints:
        ...
        # Specific ednpoints may be suppressed by setiing `<connected_endpoints_keys.key>.digital_twin.enabled` to `false`.
