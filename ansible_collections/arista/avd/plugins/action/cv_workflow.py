...
ARGUMENT_SPEC = {
    ...
    "workspace": {
        ...
        "options": {
            ...
            "max_sync_retries": {"type": "int", "required": False, "default": 2},
        },
    },
    "change_control": {
        "type": "dict",
            ...
            if work_to_do:
                ...
                result_object = await deploy_to_cv(
                    change_control=CVChangeControl(
                        avd_change_control=AvdChangeControl(**get(validated_args, "change_control", default={}))
                    ),
                    cloudvision=cloudvision,
                    ...
                    workspace=CVWorkspace(
                        avd_workspace=AvdWorkspace(**workspace_args)
                    )
                )
