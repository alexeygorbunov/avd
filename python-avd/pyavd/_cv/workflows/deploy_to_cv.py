...
async def deploy_to_cv(
    ...
    try:
        async with CVClient(
            ...
            for workspace_sync_attempt in range(0, result.workspace.max_sync_retries + 1):
                if workspace_sync_attempt > 0:
                    # All state computed against the previous mainline must be discarded and reevaluated after sync.
                ...
                if await finalize_workspace_on_cv(
                    workspace=result.workspace,
                    cv_client=cv_client,
                    devices=devices,
                    warnings=result.warnings,
